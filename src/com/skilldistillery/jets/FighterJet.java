...
public class FighterJet extends Jet implements CombatReady {
	...
	public FighterJet(String model, double speed, double range, double price, Pilot pilot, double attackPower,
			double hitPoints) {
	...
	public void fight(double damageRecieved) {
		if (damageRecieved <= hitPoints) {
			...
			System.out.printf("%s recieved %.2f damage and has %.2f HP remaining%n", getModel(), damageRecieved,
					hitPoints);
		} else {
			System.out.printf("%.2f damage is greater than %.2f HP remaining, you're going down!%n", damageRecieved,
					hitPoints);
			hitPoints = 0;
		}
	}
